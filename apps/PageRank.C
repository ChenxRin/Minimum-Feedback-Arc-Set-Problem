...
public:
    ...
    double* doPageRanking(double damping=0.0, double epsilon=0.0, double* pass_p_curr=nullptr, double* pass_p_next=nullptr){
        ...
        // newest alway stores in p_curr
        double max_pr_score=0.0;    uintT max_idx=0;
        for(size_t i=0; i<GA.n; ++i){
            if(max(max_pr_score, p_curr[i])==p_curr[i]){
                max_pr_score = p_curr[i];
                max_idx = i;
            }
        }
        cout<<"<PageRank>[check message]: iter nums:"<<iter<<", L1 norm:"<<L1_norm;
        cout<<"[(max score: "<<max_pr_score<<"), (max vertex: "<<max_idx<<")]"<<endl;
        Frontier.del(); 
...
void Compute(graph<vertex>& GA, commandLine P) {
    ...
    double* p_scores = pr_runner.doPageRanking();
    free(p_scores);
}
